'''
- file for loading the data for the experiments with data involving only numerical features
- based on https://github.com/YihanWang617/On-ell_p-Robustness-of-Ensemble-Stumps-and-Trees/blob/master/Lp_stumps_trees/data.py (which is based on https://github.com/max-andr/provably-robust-boosting/blob/master/data.py)
'''
...
def split_train_test(X_all, y_all, frac_train, current_fold=None, n_splits=None):
    """
...
def breast_cancer(current_fold=None, n_splits=None):
    """
    ...
    train: 546x10, test: 137x10 (if no cross-validation utilized)
    """
    ...
    X_train, y_train, X_test, y_test = split_train_test(X_all, y_all, frac_train=0.8, current_fold=current_fold, n_splits=n_splits)
    X_train, X_test = normalize_per_feature_0_1(X_train, X_test)

    return X_train, y_train, X_test, y_test, eps_dataset


def diabetes(current_fold=None, n_splits=None):
    """
    Taken from Kaggle:
    https://www.kaggle.com/uciml/pima-indians-diabetes-database file: diabetes.csv
    train: 614x8, test: 154x8 (if no cross-validation utilized)
    """
...
def normalize_per_feature_standardization(X_train, X_test):
    """
    We are not allowed to touch the test data, thus we do the normalization just based on the training data.
    """
...
def spambase(current_fold=None, n_splits=None):
    '''
    taken from https://archive.ics.uci.edu/ml/machine-learning-databases/spambase
    '''
    ...
    X_all = df.iloc[:, :57].to_numpy()
    y_all = df.iloc[:, 57].to_numpy()
    y_all[y_all == 0], y_all[y_all == 1] = -1, 1
    
    X_train, y_train, X_test, y_test = split_train_test(X_all, y_all, frac_train=0.8, current_fold=current_fold, n_splits=n_splits)
    X_train, X_test = normalize_per_feature_standardization(X_train, X_test)
...
def mnist_1_5(current_fold=None, n_splits=None):
    """
    train: (12163, 784), test: (2027, 784) (if no cross-validation utilized)
    """
    eps_dataset = 0.3
...
def mnist_2_6(current_fold=None, n_splits=None):
    """
    train: (11876, 784), test: (1990, 784) (if no cross-validation utilized)
    """
    eps_dataset = 0.3
...
def fmnist_sandal_sneaker(current_fold=None, n_splits=None):
    """
    Classes:
    0	T-shirt/top
    1	Trouser
    ...
    8	Bag
    9	Ankle boot
    train: (12000, 784), test: (2000, 784) (if no cross-validation utilized)
    """
    eps_dataset = 0.1
